            	...
            	$.ajax({
					...
					success : function(data){
						...
						for(var i=0;i<usersData.length;i++){
		            		...
		            		//性别
		            		if(usersStr.userRole.indexOf(selectStr) > -1){
			            		usersStr["userRole"] = changeStr(usersStr.userRole);
		            		}
		            		//会员等级
		            		if(usersStr.userGrade.indexOf(selectStr) > -1){
			            		usersStr["userGrade"] = changeStr(usersStr.userGrade);
		            		}
		            		if(usersStr.userName.indexOf(selectStr)>-1 || usersStr.userEmail.indexOf(selectStr)>-1 || usersStr.userRole.indexOf(selectStr)>-1 || usersStr.userGrade.indexOf(selectStr)>-1){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+  '<td>'+currData[i].userRole+'</td>'
			    	+  '<td>'+currData[i].userDepart+'</td>'
			    	if(currData[i].userStatus == "正常使用"){
			    	...
			    	+  '<td>'
					+    '<a class="layui-btn layui-btn-mini users_edit" data-id="'+data[i].usersId+'"><i class="iconfont icon-edit" ></i> 编辑</a>'
